...
export default function HeartHealthyTips() {
  ...
  return (
    <div className="bg-gradient-to-br from-red-50 via-white to-red-100 min-h-screen py-12 px-4">
      <motion.div
      ...
      >
        ...
        >
          <h1 className="text-4xl font-extrabold text-red-700 mb-2 text-center drop-shadow">5 Tips for a Heart-Healthy Diet</h1>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {tips.map((tip, idx) => (
            <motion.div
              key={idx}
              className="flex flex-col items-center bg-red-50 rounded-xl shadow p-6 border-t-4 border-red-200 cursor-pointer select-none"
            ...
            >
              <h2 className="text-xl font-bold text-red-600 mb-2 text-center flex items-center justify-center">
                {tip.title}
                <span className="ml-2 text-base text-red-400">{openIdx === idx ? '▲' : '▼'}</span>
              </h2>
              <motion.div
                initial={false}
